			...
			<el-row>
				<el-col :span="24">
					<el-form-item label="歌名" prop="songname">
						<el-select
							class="list_sel"
							:disabled="!isAdd||disabledForm.songname?true:false"
							v-model="form.songname" 
							placeholder="请选择歌名"
							style="width:100%;"
							@change="songnameChange">
							<el-option v-for="(item,index) in songnameLists" :label="item" :value="item">
							</el-option>
						</el-select>
					</el-form-item>
				</el-col>
				<el-col :span="24">
					<el-form-item label="音乐类型" prop="yinleleixing">
						<el-input class="list_inp" v-model="form.yinleleixing" placeholder="音乐类型"
							 type="text" 							:readonly="!isAdd||disabledForm.yinleleixing?true:false" />
					</el-form-item>
				</el-col>
...
<script setup>
	...
	//歌名回调
	const songnameChange=()=>{
		context?.$http({
			url: `follow/yinlexinxi/songname?columnValue=` + form.value.songname,
			method: 'get'
		}).then(res=>{
			//带出音乐类型字段
			if(res.data.data.yinleleixing){
				form.value.yinleleixing = res.data.data.yinleleixing
			}
			//带出歌手字段
			if(res.data.data.singer){
				form.value.singer = res.data.data.singer
			}
		})
	}
	//提交
