<template>
	<div class="app-contain" :style='{"padding":"20px 0px","margin":"20px auto 40px","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"1200px","position":"relative","justifyContent":"space-between"}'>
		<div class="back_view">
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">音乐类型</div>
					<div  class="info_text" >{{detail.yinleleixing}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">标签</div>
					<div  class="info_text" >{{detail.biaoqian}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">语言类别</div>
					<div  class="info_text" >{{detail.yuyanleibie}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">上架时间</div>
					<div  class="info_text" >{{detail.shangjiashijian}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">歌名</div>
					<div  class="info_text" >{{detail.songname}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">歌手</div>
					<div  class="info_text" >{{detail.singer}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					<div class="info_label">作词</div>
					<div  class="info_text" >{{detail.zuoci}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">作曲</div>
					<div  class="info_text" >{{detail.zuoqu}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">音乐文件</div>
					<audio class="info_audio" :src="$config.url + detail.songfile" controls="controls">
						您的浏览器不支持音频播放
					</audio>
				</div>
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth(tableName,'评分')" @click="yinlepingfenonAcross('评分','yinlepingfen','','','')" type="warning">评分</el-button>
					<el-button v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnFrontAuth('yinlexinxi','支付')" class="approval_btn" @click="payClick">支付</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('yinlexinxi','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('yinlexinxi','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
...
<script setup>
	...
	const getDetail = () => {
		context?.$http({
			...
			bannerList.value = res.data.data.gequfengmian?res.data.data.gequfengmian.split(','):[]
			title.value = res.data.data.songname
	...
	const yinlepingfenonAcross = (btnType,table,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
	...
	.detail_view{
		...
		.info_view {
			...
			.title_view {
				padding: 0 0 16px;
				margin: 0 0 12px;
				background: url(http://clfile.zggen.cn/20240314/5ca03d8a66a44982bb2f0f5852380ebd.png) repeat-x center bottom;
				display: flex;
				width: 100%;
				border-color: #66666650;
